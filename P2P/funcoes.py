...
# recebe a lista vazia de main.py e a inicializa, 
#criando uma tabela com os prefixos do hash do peer local nas linhas, mais os prefixo 0,1,2,3 nas colunas

def inicializaListaPeers(lista): 
	auxHash = ''  
	...
	for i in range(len(meuHash)+1): #tamanho do hash + 1
		for j in range(4): # cria os prefixos em colunas de 0 a 3
...
def addPeers(mutex, listaPeers, requisicao):
	for i in listaPeers: # Se o peer ja estiver na lista, retorna. Caso contrario o peer eh adicionado
		if (i == requisicao['sender']):
			return
...
def insereListaPeers(mutex, requisicao, listaDeHashPeers): #Insercao na tabela DHT 
	...
	if(cont == 0): # se nenhum dos primeiros numeros do hash coincide testa apenas com os 4 prefixos (0,1,2 e 3), ou seja, insere na primeira linha da tabela
...
def insereListaArquivos(mutex,requisicao, listaDeHashArquivos):
	...
	print hashInsert 
	#print "Hash dos Arquivos", hashInsert 
	meuHash = hashMod4(parametros.meuHash)
	cont = 0
	for i in range(len(meuHash)):
		if (meuHash[i] == hashInsert[i]):
